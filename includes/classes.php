...
// Copyright © phpSocial. All rights reserved.							\\
//----------------------------------------------------------------------\\
//======================================================================\\
...
function menu($user)
{
	...
	if ($user !== false) {
		 ...
		 */
		$links = array(
			array('profile&u=' . $user['username'], realName($user['username'], $user['first_name'], $user['last_name']), 1, 0),
...
function chatSmiles()
{
	...
	foreach ($emojis as $category => $list) {
		$output .= '<div class="emojis-list' . ($default == $category ? ' d-block' : '') . '" id="emojis-' . $category . '">';
...
class register
{
	...
	{
		if (validateAlphaNum($this->first_name) && validateAlphaNum($this->last_name)) {
			return $this->username = $this->first_name . '.' . $this->last_name;
		} elseif (validateAlphaNum($this->first_name)) {
			return $this->first_name;
		} elseif (validateAlphaNum($this->last_name)) {
	...
	function validate_values()
	{
		...
		if (!validateAlphaNum($this->username)) {
			$error[] .= 'user_alnum';
		}
...
class updateSettings
{
				...
				<div class="message-content">
					<div class="message-inner">
						...
						<div class="message-top message-no-avatar">
							...
							<div class="message-time">
								' . ((strlen($row['content']) > 65) ? mb_substr(strip_tags($row['content']), 0, 65) . '...' : strip_tags($row['content'])) . '
							</div>
						</div>
					</div>
				</div>
	...
	function createInfoPage($values, $type)
	{
		...
		global $LNG;
		$values['page_title'] = mb_substr(strip_tags($values['page_title']), 0, 64);
		$values['page_url'] = mb_substr(htmlspecialchars(strip_tags($values['page_url'])), 0, 64);
		...
		if ($type) {
			...
			$this->db->query(sprintf(
				"INSERT INTO `info_pages` (`title`, `url`, `public`, `content`) VALUES ('%s', '%s', '%s', '%s')",
...
class feed
{
		...
		$cover = '<div class="twelve columns">
					<div class="cover-container">
						<div class="cover-content">
							<a onclick="gallery(\'' . $group['cover'] . '\', \'' . ($group['id'] ?? null) . ($group['title'] ?? null) . '\', \'covers\', 0)" id="' . $group['cover'] . '"><div class="cover-image" style="background-image: url(' . permalink($this->url . '/image.php?t=c&w=900&h=200&src=' . $group['cover']) . ')">
							</div></a>
	...
	function fetchPage($page)
	{
		...
		$cover = '<div class="twelve columns">
					<div class="cover-container">
						<div class="cover-content">
							<a onclick="gallery(\'' . $page['cover'] . '\', \'' . $page['id'] . $page['name'] . '\', \'covers\', 0)" id="' . $page['cover'] . '"><div class="cover-image" style="background-image: url(' . permalink($this->url . '/image.php?t=c&w=900&h=200&src=' . $page['cover']) . ')">
							</div></a>
							<div class="cover-description">
	...
	function fetchProfile($profile)
	{
		...
		$cover = '<div class="twelve columns">
					<div class="cover-container">
						<div class="cover-content">
							<a onclick="gallery(\'' . $profile['cover'] . '\', \'' . $profile['idu'] . $profile['username'] . '\', \'covers\', 0)" id="' . $profile['cover'] . '"><div class="cover-image" style="background-image: url(' . permalink($this->url . '/image.php?t=c&w=900&h=200&src=' . $profile['cover']) . '">
							</div></a>
							<div class="cover-description">
								<div class="cover-avatar-content">
									<div class="cover-avatar">
		...
		if (!$type) {
			...
			foreach ($array as $value) {
				...
				if ($type == 2 && $for == 2) {
					...
					if ($value['event'] == 'chat') {
						$chat_snippet = ': <span class="chat-snippet">' . $this->parseMessage(mb_substr($value['message'], 0, 45)) . '...</span>';
					}
				} else {
					...
					$notification_icon = '';
					if ($value['event'] == 'chat') {
						$chat_snippet = '<br><span class="chat-snippet">' . $this->parseMessage(mb_substr($value['message'], 0, 45)) . '...</span>';
					}
				}
				if ($value['event'] == 'like') {
	...
	function parseMessage($message)
	{
		...
		if ($this->smiles) {
			foreach ($smiles as $smile => $emoji) {
				$parsedMessage = preg_replace('/\B' . preg_quote($smile) . '/mi', $emoji, $parsedMessage);
			}
		}
		...
		if ($result = $this->db->query($query)) {
			...
			if (!empty($rows)) {
				...
				if (($currentTime - $user['online']) > $this->online_time && empty($user['offline']) && $type == 0) {
					$time = date('Y-m-d H:i:s', $user['online']);
					$b = '';
					if ($this->time == '0') {
	...
	function getShares($start = null, $id = null)
	{
		global $LNG, $CONF;
		if ($start == 0) {
			$start = '';
	...
	function getActions($id, $likes = null, $comments = null, $shares = null, $update = null)
	{
		...
		if ($verify) {
			$state = '<i class="fa-solid fa-heart"></i>' . $LNG['liked'];
			$y = 2;
		} else {
			$state = '<i class="fa-regular fa-heart"></i>' . $LNG['like'];
			$y = 1;
		}
		...
		if (empty($this->id)) {
			...
			$liked = $y == 2 ? "liked" : "";
			$actions = '<a class="' . $liked . '" onclick="doLike(' . $id . ', 0)" id="doLike' . $id . '">' . $state . '</a> <a onclick="focus_form(' . $id . ')"><i class="fa-regular fa-comment"></i> ' . $LNG['comment'] . '</a><a onclick="share(' . $id . ')"><i class="fa-regular fa-paper-plane"></i> ' . $LNG['share'] . '</a>';
		}
		...
		$actions .= '<div style="margin-left: auto" class="actions_btn loader" id="action-loader' . $id . '"></div>';
	...
	function createPage($values, $type = null)
	{
		if (isset($values['token_id']) && $values['token_id'] == $_SESSION['token_id']) {
			...
			if (!validateAlphaNum($values['page_name'])) {
			...
			if ($type) {
				...
				// Create the Page
				$createPage = $this->db->query(sprintf(
					"INSERT INTO `pages` (`name`, `by`, `title`, `category`, `description`, `image`, `cover`) VALUES ('%s', '%s', '%s','%s', '%s', '%s', '%s');",
	...
	function createGroup($values, $type = null)
	{
		if (isset($values['token_id']) && $values['token_id'] == $_SESSION['token_id']) {
			...
			if (!validateAlphaNum($values['group_name'])) {
...
{ #Determine the size of the file, and print a human readable value
