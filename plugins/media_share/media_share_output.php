...
function media_share_output($values) {
    ...
    if(substr($value, 0, 3) == 'mc:') {
        return '<iframe width="100%" height="120" src="https://www.mixcloud.com/widget/iframe/?feed='.str_replace('mc:', 'https://www.mixcloud.com', $value).'&light=1" frameborder="0" class="float-left"></iframe><div class="message-divider"></div>';
    }
    ...
    if(substr($value, 0, 3) == 'sp:') {
        return '<iframe src="https://embed.spotify.com/?uri='.str_replace('sp:', 'spotify:', $value).'" width="100%" height="80" frameborder="0" allowtransparency="true" class="float-left"></iframe><div class="message-divider"></div>';
    }
    ...
    if(substr($value, 0, 3) == 'tw:') {
        return '<div class="message-type-player event-video"><iframe src="//player.twitch.tv/?channel='.str_replace('tw:/', '', $value).'&parent='.parse_url($CONF['url'], PHP_URL_HOST).'&autoplay=false" frameborder="0" scrolling="no" height="378" width="100%" class="float-left"></iframe></div><div class="message-divider"></div>';
    }

    // Dailymotion
    if(substr($value, 0, 3) == 'dm:') {
        return '<div class="message-type-player event-video"><iframe frameborder="0" width="100%" height="315" src="//www.dailymotion.com/embed/video/'.str_replace('dm:', '', $value).'" class="float-left" allowfullscreen></iframe></div><div class="message-divider"></div>';
    }

    // Metacafe
    if(substr($value, 0, 3) == 'mc:') {
        return '<div class="message-type-player event-video"><iframe width="100%" height="315" src="http://www.metacafe.com/embed/'.str_replace('mc:', '', $value).'" frameborder="0" class="float-left" allowfullscreen></iframe></div><div class="message-divider"></div>';
    }

    // Giphy
    if(substr($value, 0, 3) == 'gy:') {
        return '<div class="message-type-player event-video"><iframe src="//giphy.com/embed/'.str_replace('gy:', '', $value).'?html5=true&playOnHover=true&hideSocial=true" width="100%" height="266" frameborder="0" class="giphy-embed" class="float-left" allowfullscreen=""></iframe></div><div class="message-divider"></div>';
    }
